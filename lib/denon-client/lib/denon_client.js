...
class DenonClient extends Connection {
  ...
  _applyRegex(data) {
    ...
    _(keys).each((key) => {
      ...
      if (match != null) {
        // Copy handler to avoid mutating shared regExTable entries
        const matchResult = Object.assign({}, handler);
  ...
  sendCommand(command, parameter, hook) {
    return new Promise((resolve, reject) => {
      ...
      const cleanup = () => {
        if (listener && typeof hook === 'string') {
      ...
      if (typeof hook === 'string') {
        ...
        this.once(hook, listener);

        // Timeout after 10 seconds to prevent listener/promise leaks
        timer = setTimeout(() => {
          cleanup();
          reject(new Error(`Timeout waiting for response to: ${command}${parameter}`));
        }, 10000);
      }
